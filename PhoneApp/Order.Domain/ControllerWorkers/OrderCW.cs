...
namespace Order.Domain.ControllerWorkers
{
    public class OrderCW : IOrderCW
    {
        ...
        private IOptions<PhoneApiConfig> settings;
        public OrderCW(IOptions<PhoneApiConfig> settings)
        {
        ...
        {
            var prices = await getPrices(order.OrderDetails);

            prices = completeInfo(order, prices);

            var message = getMessage(order.Customer, prices);
        ...
        public async Task<List<OrderDetailResultDto>> getPrices(List<OrderDetailDto> details)
        {
        ...
        public List<OrderDetailResultDto> completeInfo(OrderDTO order, List<OrderDetailResultDto> detailsResults)
        {
            try
            {
        ...
        public string getMessage(CustomerDto customer, List<OrderDetailResultDto> detailsResults)
        {
